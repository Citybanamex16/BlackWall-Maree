...
exports.postLogin = async (request, response, next) => {
  ...
  try {
    if (/^CL\d{8}$/i.test(telefono)) {
      ...
      if (password) {
        const doMatch = await bcrypt.compare(password, colaborador.password)
        if (doMatch) {
        ...
        return response.render('cliente/login', buildLoginViewModel({
          error: 'Contraseña incorrecta.', telefono, passwordStep: true, mode: 'colaborador'
        }))
      }

      return response.render('cliente/login', buildLoginViewModel({
        telefono, passwordStep: true, mode: 'colaborador'
      }))
    }
...
exports.postVerifyOtp = async (request, response, next) => {
  ...
  try {
    ...
    if (client && client.codigoVerificacion === codigo) {

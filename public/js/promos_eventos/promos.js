...
const estadoPromociones = {
  ...
  promocionPendienteDesactivar: null
}
...
function configurarEventosBase () {
  ...
  if (botonConfirmarDesactivar) {
    botonConfirmarDesactivar.addEventListener('click', confirmarDesactivacion)
  }
      ...
      <div class="column is-4-desktop is-6-tablet">
        <article class="card promo-card ${activa ? '' : 'is-inactive'}">
          ...
          <footer class="card-footer promo-card-footer">
            ...
            <button
              type="button"
              class="card-footer-item promo-footer-button ${activa ? '' : 'is-disabled'}"
...
function cerrarModal (idModal) {
  ...
  if (idModal === 'modal-confirmar-desactivacion') {
    estadoPromociones.promocionPendienteDesactivar = null
  }
}
...
function solicitarDesactivacion (idPromocion) {
  const promocion = estadoPromociones.promociones.find(item => item.ID_Promocion === idPromocion)
  const mensaje = document.getElementById('mensaje-confirmar-desactivar')

  estadoPromociones.promocionPendienteDesactivar = idPromocion
...
async function confirmarDesactivacion () {
  const idPromocion = estadoPromociones.promocionPendienteDesactivar
  const botonDesactivar = document.getElementById('boton-confirmar-desactivar')

  if (!idPromocion || !botonDesactivar) {
    return
  }
  ...
  try {
    const response = await fetch(`/promos/promociones/${encodeURIComponent(idPromocion)}/desactivar`, {
      method: 'PATCH'
    })
    const result = await leerRespuestaJson(response)
    ...
    if (!response.ok || !result.success) {
      throw new Error(result.message || 'No se pudo desactivar la promoción.')
    }

    await cargarPromociones()
    mostrarModalResultado(
      'Promoción desactivada',
      result.message || 'La promoción fue desactivada correctamente.',
      'Estado de promociones'
    )
  } catch (error) {
    console.error('Error al desactivar promoción:', error)
    mostrarModalResultado('No se pudo desactivar la promoción', error.message, 'Estado de promociones')
  } finally {

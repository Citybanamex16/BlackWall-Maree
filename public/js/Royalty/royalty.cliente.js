document.addEventListener('DOMContentLoaded', () => {
  ...
  if (contenedorSellos) {
    // Obtenemos las visitas y limitamos la visualización a los sellos del nivel actual (1 a 8)
    const visitasTotales = parseInt(contenedorSellos.dataset.visitas) || 0
    let sellosGanados = visitasTotales % 8
    ...
    // Usamos 'sellos-grid' para que el CSS grid 2x4 funcione correctamente
  ...
  fetch('/royalty/royaltyUser/api/datos', {
    ...
    .then(data => {
      ...
      if (data.walletLink && btnGoogle) {
        console.log('Google wallet asignado')
        btnGoogle.href = data.walletLink
      }
      if (data.applePassUrl && btnApple) {
        console.log('Apple wallet asignado:', data.applePassUrl)
        btnApple.href = data.applePassUrl
      }
    })
    .catch(error => {

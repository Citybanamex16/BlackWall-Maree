...
--
-- Servidor: localhost
-- Tiempo de generación: 15-04-2026 a las 23:35:53
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `obtener_promociones_por_tipo` (IN `tipo_promo` VARCHAR(2))   BEGIN
    -- Lógica tipo Switch
    IF tipo_promo = 'PU' THEN
        -- Consulta para Promociones Únicas
        SELECT 
    ...
    ELSEIF tipo_promo = 'PE' THEN
        -- Consulta para Promociones de Eventos
        SELECT 
            p.Nombre AS Producto, 
            promo.Nombre AS Plantilla_Promo,
            'Evento' AS Origen
        ...
        WHERE ecp.ID_Promocion NOT IN (SELECT ID_Promocion FROM estado_royalty_da_promociones);

    ELSEIF tipo_promo = 'PR' THEN
        -- Consulta para Promociones de Royalty
        SELECT 
            p.Nombre AS Producto, 
            promo.Nombre AS Plantilla_Promo,
            'Royalty' AS Origen
